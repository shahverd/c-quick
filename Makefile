...
# Compiler and flags
# ==========================
CFLAGS := $(shell tail -n +3 compile_flags.txt | tr '\n' ' ')
...
SRC_LIB_FILES := $(foreach lib,$(SRC_LIB),$(wildcard $(LIB_DIR)/$(lib)/*.c))
OBJ_LIB_FILES := $(patsubst $(LIB_DIR)/%.c,$(BUILD_DIR)/$(LIB_DIR)/%.o,$(SRC_LIB_FILES))
...
$(BIN): $(OBJ) $(OBJ_LIB_FILES) | $(BUILD_DIR)
	gcc $(OBJ) $(OBJ_LIB_FILES) -o $@ $(LDFLAGS)
